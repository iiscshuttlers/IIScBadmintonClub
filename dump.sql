...
CREATE OR REPLACE FUNCTION "public"."admin_approve_players"("p_ids" "uuid"[], "p_approved" boolean DEFAULT true) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$ BEGIN IF NOT EXISTS (SELECT 1 FROM players WHERE id = auth.uid() AND role IN ('admin', 'master_admin')) THEN RAISE EXCEPTION 'Unauthorized'; END IF; UPDATE players SET is_approved = p_approved WHERE id = ANY(p_ids); END; $$;
...
BEGIN
  ...
  -- ΓöÇΓöÇ Advance winner ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  -- ΓöÇΓöÇ Advance loser to 3rd place match (if configured) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
BEGIN
  ...
  IF v_matches > 0 THEN
    RAISE EXCEPTION 'Guest has % match(es) on record ΓÇö claim the profile instead of deleting.', v_matches;
  END IF;
...
BEGIN
  IF OLD.status = 'pending' AND NEW.status = 'confirmed' THEN
    notif_title   := CASE WHEN NEW.is_friendly THEN '≡ƒÅ╕ Friendly Match Accepted' ELSE '≡ƒÅ╕ Tournament Match Accepted' END;
...
BEGIN
  ...
  notif_title   := CASE WHEN is_friendly THEN '≡ƒÅ╕ Friendly Match Logged' ELSE '≡ƒÅ╕ Tournament Match Logged' END;
...
BEGIN
  ...
  -- ΓöÇΓöÇ Dedup: return existing pending match if submitted in the last 2 hours ΓöÇΓöÇ
...
BEGIN
  ...
  UPDATE tournament_matches SET
    ...
    scored_by    = COALESCE(scored_by, auth.uid()),
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS DISTINCT FROM 'confirmed') THEN
    -- Skip Hybrid ΓÇö no ELO, no category record
...
CREATE POLICY "Admins and Umpires can insert matches" ON "public"."matches" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."players"
  WHERE (("players"."id" = "auth"."uid"()) AND ("players"."role" = ANY (ARRAY['admin'::"text", 'master_admin'::"text", 'umpire'::"text"]))))));
...
CREATE POLICY "Admins and Umpires can update matches" ON "public"."matches" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."players"
  WHERE (("players"."id" = "auth"."uid"()) AND ("players"."role" = ANY (ARRAY['admin'::"text", 'master_admin'::"text", 'umpire'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."players"
  WHERE (("players"."id" = "auth"."uid"()) AND ("players"."role" = ANY (ARRAY['admin'::"text", 'master_admin'::"text", 'umpire'::"text"]))))));
...
CREATE POLICY "Admins can delete matches" ON "public"."matches" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."players"
  WHERE (("players"."id" = "auth"."uid"()) AND ("players"."role" = ANY (ARRAY['admin'::"text", 'master_admin'::"text"]))))));
...
CREATE POLICY "insert own vote" ON "public"."live_match_votes" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "tm_admin_read" ON "public"."tournament_matches" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."players"
  WHERE (("players"."id" = "auth"."uid"()) AND ("players"."role" = ANY (ARRAY['admin'::"text", 'master_admin'::"text", 'umpire'::"text"]))))));



CREATE POLICY "tm_admin_write" ON "public"."tournament_matches" USING ((EXISTS ( SELECT 1
   FROM "public"."players"
  WHERE (("players"."id" = "auth"."uid"()) AND ("players"."role" = ANY (ARRAY['admin'::"text", 'master_admin'::"text", 'umpire'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."players"
  WHERE (("players"."id" = "auth"."uid"()) AND ("players"."role" = ANY (ARRAY['admin'::"text", 'master_admin'::"text", 'umpire'::"text"]))))));
