...
async function main() {
  // Check current roles for all players
  const { data, error } = await supabase.from('players').select('id, full_name, email, role').not('role', 'is', null);
  console.log('Players with roles:', data?.map(p => ({ name: p.full_name, email: p.email, role: p.role })));
