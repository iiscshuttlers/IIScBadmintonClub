...
export interface SensorAnalytics {
  ...
  fastestShotIntervalMs: number | null;
...
export function usePlayerPersonal(playerId: string | undefined) {
  return useQuery({
    ...
    enabled: !!playerId,
    staleTime: 2 * 60 * 1000,
    queryFn: async (): Promise<{ matches: PersonalMatch[]; motion: MotionSummary | null; motionMatches: MotionMatch[]; sensorAnalytics: SensorAnalytics[] }> => {
      if (!playerId) return { matches: [], motion: null, motionMatches: [], sensorAnalytics: [] };

      const { data, error } = await supabase
        .from("tournament_matches")
        .select(
          "id, tournament_id, category, round_name, score, status, winner_side, scored_at, started_at, ended_at, " +
      ...
      const matches: PersonalMatch[] = rows.map((m) => {
      ...
      // Query tournament motion/sensors if the player has any matches
      let tMotionRows: any[] = [];
      let tSensorRows: any[] = [];
      if (matchIds.length > 0) {
        const [mRes, sRes] = await Promise.all([
          supabase.from("match_motion_stats").select("*").eq("match_source", "tournament").in("match_id", matchIds).then(res => res),
          supabase.from("match_sensor_analytics").select("*").eq("match_source", "tournament").in("match_id", matchIds).then(res => res)
        ]);
        if (mRes.data) tMotionRows = mRes.data;
        if (sRes.data) tSensorRows = sRes.data;
      }

      // Also query self-tracked practice motion/sensors for this player
      const [pMRes, pSRes] = await Promise.all([
        supabase.from("match_motion_stats").select("*").eq("match_source", "practice").eq("recorded_by", playerId).then(res => res),
        supabase.from("match_sensor_analytics").select("*").eq("match_source", "practice").eq("player_id", playerId).then(res => res)
      ]);
      const pMotionRows = pMRes.data || [];
      const pSensorRows = pSRes.data || [];

      const motionRows = [...tMotionRows, ...pMotionRows];
      const sensorRows = [...tSensorRows, ...pSensorRows];
      ...
      if (motionRows.length > 0) {
        ...
        motionMatches = motionRows
          .map((r) => {
            ...
            const isPractice = r.match_source === "practice";
            let tournamentName = "Practice Session";
            let scoredAt = null;
            let startedAt: string | null = null;
            let endedAt: string | null = null;
            let won = null;
            let group: PersonalMatch["group"] = "Singles";

            if (isPractice) {
              scoredAt = r.created_at || new Date().toISOString();
            } else {
      ...
      if (sensorRows && sensorRows.length > 0) {
         sensorAnalytics = sensorRows.map(r => ({
           ...
           accelStd: r.accel_std || 0,
           gyroStd: r.gyro_std || 0,
           avgShotIntervalMs: r.avg_shot_interval_ms ?? null,
           fastestShotIntervalMs: r.fastest_shot_interval_ms ?? null,
           directionChanges: r.direction_changes || 0,
         }));
      }
