...
export function ChallengeHubTab({ currentUser }: { currentUser: any }) {
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-sm text-slate-800 dark:text-slate-200">
                      {c.challenger.full_name} <span className="text-slate-400 font-normal">challenged you to</span> <span className="text-orange-500">{c.format}</span>
                    </div>
                    {c.scheduled_time && (
                    ...
                    {c.message && <div className="text-xs italic text-slate-500 mt-1">"{c.message}"</div>}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-xs text-slate-800 dark:text-slate-200">
                      Waiting on {c.challenged.full_name} <span className="text-slate-400 font-normal">({c.format})</span>
                    </div>
                  </div>
                </div>
                <button onClick={() => handleAction(c.id, "cancelled")} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-full transition">
      ...
      {/* History */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
        <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-4">Challenge History</h3>
        {history.length === 0 ? (
          <div className="text-center text-slate-400 text-sm py-4">No challenge history yet.</div>
        ) : (
