    ...
    <div className="bg-slate-900 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden border border-slate-800">
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-foreground uppercase tracking-wider">Top Rivalries</h2>
            <p className="text-sm font-medium text-muted-foreground">The most hotly contested matchups in the club (Min. 5 matches)</p>
          </div>
        </div>
          ...
          {rivalries.map((r, i) => (
            <div key={i} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 hover:bg-slate-800 transition-colors group">
              <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground bg-slate-900/50 px-2 py-1 rounded-md">
                  {r.total} Matches
                </span>
                {r.closeness <= 10 && (
                ...
                <Link href={`/player/${r.p1}`} className="flex flex-col items-center gap-2 group-hover:scale-105 transition-transform flex-1">
                  <div className="w-14 h-14 rounded-full bg-slate-700 overflow-hidden ring-2 ring-slate-600 group-hover:ring-orange-500 transition-colors">
                    {r.p1Avatar ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-xl font-bold text-muted-foreground">{r.p1Name[0]}</div>
                    )}
                  </div>
                  <span className="text-xs font-bold text-foreground text-center line-clamp-1">{r.p1Name.split(" ")[0]}</span>
                ...
                <Link href={`/player/${r.p2}`} className="flex flex-col items-center gap-2 group-hover:scale-105 transition-transform flex-1">
                  <div className="w-14 h-14 rounded-full bg-slate-700 overflow-hidden ring-2 ring-slate-600 group-hover:ring-orange-500 transition-colors">
                    {r.p2Avatar ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-xl font-bold text-muted-foreground">{r.p2Name[0]}</div>
                    )}
                  </div>
                  <span className="text-xs font-bold text-foreground text-center line-clamp-1">{r.p2Name.split(" ")[0]}</span>
