      ...
      <DropdownMenuContent align="end" className="w-80 p-0 border-0 overflow-hidden rounded-2xl shadow-2xl shadow-slate-300/40 dark:shadow-black/60">
        ...
        <div className="bg-slate-50 dark:bg-slate-950 p-2.5 space-y-2 max-h-[70vh] overflow-y-auto">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
            <DropdownMenuItem onSelect={() => setLocation("/profile/setup")} className="cursor-pointer font-semibold rounded-none text-slate-700 dark:text-slate-300 focus:text-slate-900 dark:focus:text-white focus:bg-slate-50 dark:focus:bg-slate-800 px-3 py-2.5 gap-2.5">
              <User className="w-4 h-4 text-slate-400" /> Edit Profile
            </DropdownMenuItem>
            <DropdownMenuItem onSelect={() => setLocation("/profile/password")} className="cursor-pointer font-semibold rounded-none text-slate-700 dark:text-slate-300 focus:text-slate-900 dark:focus:text-white focus:bg-slate-50 dark:focus:bg-slate-800 px-3 py-2.5 border-t border-slate-100 dark:border-slate-800 gap-2.5">
              <Lock className="w-4 h-4 text-slate-400" /> Change Password
            </DropdownMenuItem>
            <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setIsPreferencesOpen(true); }} className="cursor-pointer font-semibold rounded-none text-slate-700 dark:text-slate-300 focus:text-slate-900 dark:focus:text-white focus:bg-slate-50 dark:focus:bg-slate-800 px-3 py-2.5 border-t border-slate-100 dark:border-slate-800 gap-2.5">
              <Settings className="w-4 h-4 text-slate-400" /> App Preferences
            </DropdownMenuItem>
            <DropdownMenuItem onSelect={() => setLocation("/join?add_account=true")} className="cursor-pointer font-semibold rounded-none text-slate-700 dark:text-slate-300 focus:text-slate-900 dark:focus:text-white focus:bg-slate-50 dark:focus:bg-slate-800 gap-2.5 px-3 py-2.5 border-t border-slate-100 dark:border-slate-800">
...
{savedAccounts.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              {savedAccounts.map((acc) => (
                <DropdownMenuItem key={acc.id} className="cursor-pointer font-medium rounded-none focus:text-slate-900 dark:focus:text-white focus:bg-slate-50 dark:focus:bg-slate-800 px-3 py-2.5 border-b border-slate-50 dark:border-slate-800/50 last:border-b-0" onClick={async () => { await switchAccount(acc); }}>
