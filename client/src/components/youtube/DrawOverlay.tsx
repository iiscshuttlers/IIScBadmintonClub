  ...
  return (
    <div className="absolute inset-0 z-40 bg-black/10">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/60 backdrop-blur-md px-4 py-2 rounded-full shadow-xl pointer-events-auto">
        <span className="text-foreground font-bold text-sm mr-2 tracking-wide">DRAW MODE</span>
        <button onClick={(e) => { e.stopPropagation(); setDrawLines([]); setCurrentLine([]); }} className="text-foreground/80 hover:text-foreground bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-full text-xs font-bold transition-colors">Clear</button>
