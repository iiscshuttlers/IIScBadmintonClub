...
export default function Navigation() {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b-[3px] border-emerald-500 transition-all duration-300 ${scrolled && !isOpen ? "shadow-xl" : "shadow-sm"}`}
    >
      <div
        className={`container mx-auto px-4 transition-all duration-300 ${scrolled ? "py-2" : "py-3"}`}
      >
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex gap-2 items-center">
            ...
            <Link href="/invicta">
              <Button
                variant="outline"
                className="border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 font-bold px-4 py-2 rounded-xl h-10 flex items-center gap-2 transition-all ml-2 shadow-sm"
              >
            ...
            {authLoading ? (
              ...
              <DropdownMenu>
                ...
                >
                  <Link
                    href={
                      myPlayerId ? `/player/${myPlayerId}` : "/profile/setup"
                    }
                  >
          ...
          <div className="lg:hidden absolute top-full left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-xl max-h-[85vh] overflow-y-auto">
            <div className="container mx-auto px-4 py-4 flex flex-col gap-2">
              <Link href="/">
                <Button
                  variant="ghost"
                  className={`w-full justify-start text-lg py-6 ${isActive("/") ? "text-emerald-600 font-bold bg-emerald-50 dark:bg-emerald-900/30" : ""}`}
              ...
              {TOP_LEVEL_LINKS.map((link) => (
                <Link key={link.href} href={link.href}>
                  <Button
                    variant="ghost"
                    className={`w-full justify-start text-lg py-6 ${isActive(link.href) ? "text-emerald-600 font-bold bg-emerald-50 dark:bg-emerald-900/30" : "text-slate-600 dark:text-slate-400"}`}
                    onClick={() => setIsOpen(false)}
                  >
                    {link.label}
                  </Button>
                ...
                {authLoading ? (
                  ...
                  <div className="flex flex-col gap-1 mt-2 border-t border-slate-100 dark:border-slate-800 pt-2">
                    ...
                    >
                      <Button
                        variant="ghost"
                        className="w-full justify-between text-emerald-600 dark:text-emerald-400 font-medium hover:bg-emerald-50 dark:hover:bg-emerald-950/50"
                        onClick={() => setIsOpen(false)}
                      >
                        <div className="flex items-center">
                          <User className="mr-2 h-4 w-4" /> My Profile
                    ...
                    >
                      <Button
                        variant="ghost"
                        className="w-full justify-start text-emerald-600 dark:text-emerald-400 font-medium hover:bg-emerald-50 dark:hover:bg-emerald-950/50"
                        onClick={() => setIsOpen(false)}
                      >
                        <Settings className="mr-2 h-4 w-4" /> Edit Profile &
                        Password
                    ...
                    {isAdmin && (
                      <Link href="/admin">
                        <Button
                          variant="ghost"
                          className="w-full justify-start text-purple-600 font-medium hover:bg-purple-50 dark:hover:bg-purple-950/50"
                          onClick={() => setIsOpen(false)}
                        >
                          <Settings className="mr-2 h-4 w-4" /> Site Admin
                        </Button>
                    ...
                    <div className="pt-2 mt-2 border-t border-slate-100 dark:border-slate-800">
                      <Link href="/join?add_account=true">
                        <Button
                          variant="ghost"
                          className="w-full justify-start text-slate-600 dark:text-slate-400 font-medium hover:bg-slate-50 dark:hover:bg-slate-800"
                          onClick={() => setIsOpen(false)}
                        >
                          <UserPlus className="mr-2 h-4 w-4" /> Add Account
                        </Button>
