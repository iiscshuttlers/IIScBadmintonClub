...
export default function InvictaRegistrationForm({
  onClose,
}: {
  onClose?: () => void;
}) {
  ...
  const totalAmount = selectedCats.reduce(
    (sum, catId) => sum + CATEGORIES.find((c) => c.id === catId)!.price,
    0,
  );
  ...
  const submitRegistration = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (dbError)
        throw new Error("Failed to submit entry: " + dbError.message);
    ...
    return (
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 text-center">
        ...
        <p className="text-slate-500 mb-6">
          You must be logged in to register for Invicta.
        </p>
        <button
  ...
  if (success) {
    return (
      <motion.div
        ...
        className="bg-emerald-50 dark:bg-emerald-950/20 border-2 border-emerald-500 rounded-3xl p-10 text-center relative overflow-hidden"
      >
        ...
        <h3 className="text-3xl font-black mb-3 text-emerald-900 dark:text-emerald-400 relative z-10">
          Registration Received!
        </h3>
        <p className="text-emerald-700 dark:text-emerald-500 mb-8 max-w-sm mx-auto relative z-10">
          Your payment is being verified by our team. You will be notified once
          your entry is confirmed.
        </p>
        {onClose && (
          <button
            onClick={onClose}
            className="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl shadow-lg relative z-10"
          >
            Close Form
          </button>
        )}
  ...
  return (
    <form
      ...
      className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-10 shadow-xl relative text-left"
    >
      {onClose && (
        <button
          type="button"
      ...
      <p className="text-slate-500 dark:text-slate-400 mb-8">
        Select your events and upload payment details below.
      </p>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-5 h-5 rounded-md flex items-center justify-center border transition-colors ${isSelected ? "bg-emerald-500 border-emerald-500 text-white" : "border-slate-300 dark:border-slate-600"}`}
                  >
      ...
      <AnimatePresence>
        {selectedCats.some((c) => c.endsWith("D")) && (
          ...
          >
            ...
            {selectedCats
              .filter((c) => c.endsWith("D"))
              .map((catId) => (
                <div key={catId} className="flex flex-col gap-1.5">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                    {CATEGORIES.find((c) => c.id === catId)?.label} Partner
                  </span>
                  <input
                    required
        ...
        <div className="space-y-6 mb-8 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Please UPI <strong>₹{totalAmount}</strong> to{" "}
