...
export function ChangelogViewer() {
  ...
  if (changelog.length === 0) {
    return (
      <div className="p-10 text-center text-muted-foreground dark:text-muted-foreground bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
      ...
      <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-300 dark:before:via-slate-700 before:to-transparent">
        {changelog.map((release, idx) => (
          <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
            <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-slate-50 dark:border-slate-950 bg-primary text-primary-foreground shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow">
            ...
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                <h4 className="font-black text-lg text-primary dark:text-primary">Version {release.versionName}</h4>
                <span className="text-xs font-bold text-muted-foreground dark:text-muted-foreground bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg">Build {release.versionCode}</span>
              </div>
              <time className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4">
              ...
              <ul className="space-y-2">
                {release.changes.map((change: string, i: number) => (
                  <li key={i} className="text-sm text-muted-foreground dark:text-slate-300 flex items-start gap-2">
