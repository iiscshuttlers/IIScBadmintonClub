      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-2 justify-center">
          {tags.map((tag) => (
            <div
              key={tag.id}
              className="flex items-center gap-1.5 bg-white/15 hover:bg-white/25 backdrop-blur-sm border border-white/20 text-foreground text-xs font-bold px-3 py-1 rounded-full cursor-pointer transition-colors group"
            ...
            >
              ...
              {isAdmin && (
                <button
                  className="ml-1 text-foreground/40 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
      ...
      {isAdmin && pendingTags.length > 0 && (
        <div className="flex flex-col items-center justify-center gap-1.5 mt-2 bg-slate-900/60 border border-amber-500/30 p-2.5 rounded-2xl w-full max-w-sm backdrop-blur-md">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {pendingTags.map((pt) => (
              <div key={pt.id} className="flex items-center gap-1.5 bg-amber-500/20 border border-amber-500/40 text-foreground text-xs font-bold px-3 py-1 rounded-full">
                {pt.name}
      ...
      {currentUserProfile && !isAdmin && (() => {
        ...
        return (
          <button
            ...
            className="flex items-center gap-1.5 mt-2 text-foreground/80 hover:text-foreground bg-white/5 hover:bg-white/10 border border-white/20 hover:border-white/40 px-3 py-1.5 rounded-full text-[11px] font-bold shadow-lg transition-all"
          >
      ...
      {isAdmin && (
        <div className="flex items-center gap-2 relative">
          <div className="relative mt-2">
            <button
              className="flex items-center gap-1.5 text-foreground/50 hover:text-foreground text-xs font-bold px-3 py-1.5 rounded-full border border-white/20 hover:border-white/40 hover:bg-white/10 transition-all"
