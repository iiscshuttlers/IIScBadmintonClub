...
const FORMAT_COLORS: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  MS: { bg: 'bg-blue-50 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-400', border: 'border-blue-100 dark:border-blue-900/50 dark:border-blue-900/50', dot: 'bg-blue-500' },
  WS: { bg: 'bg-pink-50 dark:bg-pink-900/30', text: 'text-pink-700 dark:text-pink-400', border: 'border-pink-100 dark:border-pink-900/50', dot: 'bg-pink-500' },
  MD: { bg: 'bg-emerald-50 dark:bg-emerald-900/30', text: 'text-emerald-700', border: 'border-emerald-100 dark:border-emerald-900/50 dark:border-emerald-900/50', dot: 'bg-emerald-500' },
  WD: { bg: 'bg-purple-50 dark:bg-purple-900/30', text: 'text-purple-700 dark:text-purple-400', border: 'border-purple-100 dark:border-purple-900/50', dot: 'bg-purple-500' },
  XD: { bg: 'bg-orange-50 dark:bg-orange-900/30', text: 'text-orange-700 dark:text-orange-400', border: 'border-orange-100 dark:border-orange-900/50', dot: 'bg-orange-500' },
};

const POOL_COLORS: Record<string, string> = {
  Finals: 'from-amber-50 to-yellow-50 dark:from-amber-950/30 dark:to-yellow-950/30 border-amber-200 dark:border-amber-900/50',
  'League A': 'from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 border-blue-100 dark:border-blue-900/50 dark:border-blue-900/50',
  'League B': 'from-emerald-50 to-teal-50 dark:from-emerald-950/30 dark:to-teal-950/30 border-emerald-100 dark:border-emerald-900/50 dark:border-emerald-900/50',
  League: 'from-slate-50 dark:from-slate-900 to-gray-50 border-slate-200 dark:border-slate-700',
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 sm:px-4 py-6 sm:py-8 space-y-6 sm:space-y-8 font-sans text-slate-900 dark:text-white">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-slate-900 dark:text-white">Match Schedule</h2>
            ...
            <div key={time} className="relative flex flex-col xs:flex-row gap-3 sm:gap-4 md:gap-8">
              ...
              <div className="flex-1 space-y-4">
                {matches.map((m, idx) => {
                  ...
                  const formatStyle = FORMAT_COLORS[m.format] || { bg: 'bg-slate-50 dark:bg-slate-800', text: 'text-slate-600 dark:text-slate-300 dark:text-slate-600', border: 'border-slate-100 dark:border-slate-800', dot: 'bg-slate-400' };
                  const poolGradient = POOL_COLORS[m.Pool] || POOL_COLORS[m.Round] || 'from-white to-white dark:from-slate-900 dark:to-slate-900 border-slate-200 dark:border-slate-700';
                      ...
                      <div className="px-4 sm:px-6 py-4 sm:py-6">
                        <div className="flex flex-col md:grid md:grid-cols-[1fr_auto_1fr] items-center gap-4 sm:gap-8">
                          ...
                          <div className={`w-full flex items-center md:justify-end gap-3 sm:gap-4 ${p1Won ? 'md:scale-105 md:origin-right' : ''}`}>
                            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-base sm:text-lg font-bold flex-shrink-0 ${p1Won ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 ring-2 ring-amber-200 dark:ring-amber-900/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
                              {p1List.length > 1 ? <Users size={18} className="sm:w-5 sm:h-5" /> : p1List[0]?.charAt(0)}
                            </div>
                            <div className="text-left md:text-right flex-1">
                              {p1List.map((player, pIdx) => (
                                <p key={pIdx} className={`text-sm sm:text-base font-bold leading-tight ${p1Won ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-200 dark:text-slate-700'}`}>
                                  {player}
                                </p>
                              ))}
                          ...
                          <div className="hidden md:flex flex-col items-center justify-center gap-4">
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded border border-slate-100 dark:border-slate-800">
                              <span className="text-[10px] font-black text-slate-300 dark:text-slate-600 tracking-[0.3em] uppercase">VS</span>
                            </div>
                            <div className="h-10 w-px bg-slate-200 dark:bg-slate-700" />
                          </div>
                          ...
                          <div className="md:hidden w-full flex items-center gap-3">
                            ...
                            <span className="text-[8px] font-black text-slate-200 dark:text-slate-700 tracking-[0.2em] uppercase">VS</span>
                            <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800" />
                          </div>

                          {/* Team 2 */}
                          <div className={`w-full flex items-center gap-3 sm:gap-4 ${p2Won ? 'md:scale-105 md:origin-left' : ''}`}>
                            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-base sm:text-lg font-bold flex-shrink-0 ${p2Won ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 ring-2 ring-amber-200 dark:ring-amber-900/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
                              {p2List.length > 1 ? <Users size={18} className="sm:w-5 sm:h-5" /> : p2List[0]?.charAt(0)}
                            </div>
                            <div className="text-left flex-1">
                              {p2List.map((player, pIdx) => (
                                <p key={pIdx} className={`text-sm sm:text-base font-bold leading-tight ${p2Won ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-200 dark:text-slate-700'}`}>
                                  {player}
                                </p>
                              ))}
                            ...
                            {sets.map((set, sIdx) => {
                              ...
                              return (
                                <div key={sIdx} className="flex flex-col items-center gap-1 sm:gap-1.5">
                                  <span className="text-[7px] sm:text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Set {sIdx + 1}</span>
                                  <div className="flex items-center bg-slate-900 rounded-lg sm:rounded-xl overflow-hidden shadow-md sm:shadow-lg border border-white/10">
                                    <div className={`px-3 sm:px-4 py-1.5 sm:py-2 text-base sm:text-xl font-black font-mono min-w-[40px] sm:min-w-[50px] text-center ${s1Won ? 'text-amber-400 bg-white dark:bg-slate-900/5' : 'text-slate-400 dark:text-slate-500'}`}>
                                      {set.s1}
                                    </div>
                                    <div className="w-px h-4 sm:h-6 bg-white dark:bg-slate-900/10" />
                                    <div className={`px-3 sm:px-4 py-1.5 sm:py-2 text-base sm:text-xl font-black font-mono min-w-[40px] sm:min-w-[50px] text-center ${s2Won ? 'text-amber-400 bg-white dark:bg-slate-900/5' : 'text-slate-400 dark:text-slate-500'}`}>
                                      {set.s2}
                                    </div>
                                  </div>
                                </div>
