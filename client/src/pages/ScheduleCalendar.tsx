...
import { CalendarDays, MapPin, Clock, ArrowRight, ExternalLink } from "lucide-react";
...
export default function ScheduleCalendar() {
  ...
  const selectedEvents = events.filter(e => {
    ...
    if (e.endDate && selectedDateStr >= e.date && selectedDateStr <= e.endDate) return true;
  ...
  const deadlineDates = events.filter(e => e.registrationDeadline).map(e => new Date(e.registrationDeadline as string));
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
                              ...
                              <div className="space-y-1.5 p-1">
                                {dayEvents.map((e, i) => (
                                  <div key={i} className="flex items-center gap-2">
                                    <span className={`w-2 h-2 rounded-full ${e.type === 'holiday' ? getHolidayColor(e.title).solid : 'bg-emerald-500'}`} />
                                    <span className="font-semibold text-xs">{e.title}</span>
                                    {e.type !== 'holiday' && e.date === dateStr && (
                                      <span className="text-[10px] bg-blue-500/20 text-blue-300 px-1.5 py-0.5 rounded uppercase font-bold">Starts</span>
                                    )}
                                  </div>
                                ))}
                              </div>
            ...
            <Card className="rounded-3xl shadow-lg border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 mt-6 overflow-hidden">
              <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 dark:text-slate-200">Coming Up</h3>
              </div>
              <div className="p-5 space-y-4">
                {loading ? (
                  <div className="animate-pulse flex gap-3"><div className="w-10 h-10 bg-slate-200 rounded" /><div className="flex-1 space-y-2"><div className="h-4 bg-slate-200 rounded w-3/4" /><div className="h-3 bg-slate-200 rounded w-1/2" /></div></div>
                ) : upcomingEvents.slice(0, 3).map((e, idx) => {
                  const d = new Date(e.date);
                  const isHoliday = e.type === 'holiday';
                  const holidayColor = isHoliday ? getHolidayColor(e.title) : null;
                  const isMultiDayStart = e.endDate && e.date !== e.endDate;
                  return (
                  <div key={idx} className="flex gap-4">
                    <div className={`flex flex-col items-center justify-center w-14 shrink-0 rounded-xl text-center p-1.5 border ${
                      isHoliday 
                      ...
                      <span className={`text-[9px] font-black uppercase ${isHoliday ? holidayColor?.text : isMultiDayStart ? 'text-blue-600' : 'text-emerald-600'}`}>{d.toLocaleString('default', { weekday: 'short' })}</span>
                      <span className={`text-xl font-black leading-none my-0.5 ${isHoliday ? holidayColor?.textDark : isMultiDayStart ? 'text-blue-700 dark:text-blue-400' : 'text-emerald-700 dark:text-emerald-400'}`}>{d.getDate()}</span>
                      <span className={`text-[10px] font-bold uppercase ${isHoliday ? holidayColor?.text : isMultiDayStart ? 'text-blue-500' : 'text-emerald-500'}`}>{d.toLocaleString('default', { month: 'short' })}</span>
                    </div>
                    <div className="flex flex-col justify-center">
                      <h4 className="font-bold text-sm text-slate-800 dark:text-slate-200 line-clamp-1">{e.title}</h4>
                      {e.time ? (
                        <div className="text-xs text-slate-500 flex items-center gap-1 mt-1"><Clock className="w-3 h-3" /> {e.time}</div>
                      ) : isHoliday ? (
                        <div className={`text-xs ${holidayColor?.text} flex items-center gap-1 mt-1 font-semibold`}>Holiday</div>
                      ) : isMultiDayStart ? (
                        <div className="text-xs text-blue-500 flex items-center gap-1 mt-1 font-semibold">Tournament Starts</div>
                      ) : null}
                    </div>
                  </div>
                  );
                ...
                {selectedEvents.length > 0 ? (
                  <div className="space-y-6">
                    {selectedEvents.map((event, idx) => {
                      ...
                      const holidayColor = isHoliday ? getHolidayColor(event.title) : null;
                      return (
                      <div key={idx} className={`p-4 md:p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden group ${isHoliday ? 'opacity-80' : ''}`}>
                        <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${isHoliday ? holidayColor?.solid : 'bg-emerald-500'}`} />
                        <div className="flex items-center gap-2 mb-3 flex-wrap">
                          <h3 className="text-xl font-bold text-slate-900 dark:text-white">{event.title}</h3>
                          {isHoliday && (
                            <span className={`px-2 py-0.5 rounded-full ${holidayColor?.badge} text-xs font-bold uppercase`}>
                              Holiday
                            </span>
                          )}
                          {event.endDate && event.date === selectedDateStr && !isHoliday && (
                          ...
                          {event.endDate && event.endDate === selectedDateStr && !isHoliday && (
                        ...
                        <div className="flex flex-wrap gap-4 mb-4">
                          {event.time && (
                            <div className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 dark:text-slate-300">
                              <Clock className="w-4 h-4 text-emerald-500" /> {event.time}
                            </div>
                          )}
                          {event.location && (
                            <div className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 dark:text-slate-300">
                              <MapPin className="w-4 h-4 text-emerald-500" /> {event.location}
                            </div>
                          )}
                        </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300">No Events Scheduled</h3>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 max-w-sm">
                      There are no matches or practice sessions scheduled for this date. Enjoy the rest!
                    </p>
                  </div>
                )}
              </div>
