...
const fadeUp = {
  ...
  visible: (i: number) => ({ opacity: 1, y: 0, transition: { duration: 0.4, delay: i * 0.06, ease: 'easeOut' as const } }),
};
  ...
  const loadHolidays = useCallback(() => {
    fetchSiteData<Holiday[]>("holidays", "holidays.json")
      .then(data => {
        ...
        setNextHoliday(data.find((h: any) => h.date >= today) || null);
      })
              ...
              <div className="h-full rounded-3xl border-2 border-emerald-100 dark:border-emerald-900/50 shadow-md bg-white dark:bg-slate-800 overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h2 className="text-2xl font-black text-blue-900 dark:text-white">Indoor Courts</h2>
                  </div>
                  <p className="text-gray-600 dark:text-slate-400 leading-relaxed mb-6">
                    Three professional-grade wooden courts with synthetic mat flooring and modern lighting, designed for both training and competitive play.
                  </p>
              ...
              <div className="h-full rounded-3xl border-2 border-blue-100 dark:border-blue-900/50 shadow-md bg-white dark:bg-slate-800 overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h2 className="text-2xl font-black text-blue-900 dark:text-white">Operating Hours</h2>
                  </div>
                  <div className="space-y-3 mb-4">
                    {schedule.map((slot, idx) => (
                      ...
                      >
                        <div>
                          <p className={`font-bold text-sm ${idx === 0 ? 'text-emerald-800 dark:text-emerald-300' : 'text-red-700 dark:text-red-400'}`}>
                            {slot.day}
                          </p>
                          <p className="text-xs text-gray-500 dark:text-slate-500 mt-0.5">{slot.note}</p>
                        </div>
                        <span className={`font-black text-sm tabular-nums ${idx === 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                          {slot.hours}
                        </span>
                      </div>
                    ))}
            ...
            >
              ...
              <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black text-white/60 uppercase tracking-widest">Next Closure</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {holidays.map((h, i) => {
                ...
                return (
                  ...
                  >
                    <p className={`text-xs font-bold tabular-nums ${isNext ? 'text-amber-600 dark:text-amber-400' : isPast ? 'text-slate-400 dark:text-slate-500' : 'text-gray-500 dark:text-slate-400'}`}>
                      {h.date}
                    </p>
                    <p className={`text-xs font-semibold mt-1.5 leading-snug ${isNext ? 'text-amber-800 dark:text-amber-300' : isPast ? 'text-slate-400 dark:text-slate-500' : 'text-blue-900 dark:text-slate-200'}`}>
                      {h.name}
                    </p>
                    {isNext && <span className="mt-2 inline-block text-[10px] font-black uppercase tracking-wider text-amber-600 dark:text-amber-400">Next →</span>}
                    {isPast && <span className="mt-2 inline-block text-[10px] text-slate-400 dark:text-slate-600">Past</span>}
                  </motion.div>
                );
              })}
            </div>
              ...
              <div className="h-full rounded-3xl border border-slate-100 dark:border-slate-700 shadow-md bg-white dark:bg-slate-800 overflow-hidden">
                ...
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h2 className="text-2xl font-black text-blue-900 dark:text-white">Location</h2>
                  </div>
                  ...
                  <p className="text-gray-500 dark:text-slate-500 text-sm mb-6">
                    Located within the IISc campus with easy access, ample parking, and excellent connectivity.
                  </p>
                  <a
              ...
              <div className="h-full rounded-3xl border border-slate-100 dark:border-slate-700 shadow-md bg-white dark:bg-slate-800 overflow-hidden">
                ...
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h2 className="text-2xl font-black text-blue-900 dark:text-white">Membership & Access</h2>
                  </div>
                  ...
                  <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/50 rounded-2xl p-5 mb-5">
                    <h3 className="font-black text-emerald-700 dark:text-emerald-400 text-sm uppercase tracking-wider mb-2">IISc Members</h3>
                    <p className="text-gray-600 dark:text-slate-400 text-sm leading-relaxed">
                      Students, faculty, staff, and eligible members can access the badminton facilities through Gymkhana membership.
                    </p>
                  </div>
                  ...
                  <ul className="space-y-3">
                    {[
                      ...
                      'Join the IISc badminton community',
                      'Access to club WhatsApp groups',
                    ].map((item, i) => (
                      <li key={i} className="flex gap-3 items-start">
                        <CheckCircle className="w-5 h-5 text-emerald-500 mt-0.5 flex-shrink-0" />
