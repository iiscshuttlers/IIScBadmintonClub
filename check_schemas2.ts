...
async function main() {
  const { data: tm } = await supabase.from('tournament_matches').select('*').limit(1);
  console.log('tournament_matches columns:', Object.keys(tm?.[0] || {}));
  
  const { data: t } = await supabase.from('tournaments').select('*').limit(1);
  console.log('tournaments columns:', Object.keys(t?.[0] || {}));
}
