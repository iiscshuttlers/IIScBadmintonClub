...
BEGIN
  ...
  SELECT v_player_id::text = ANY(kudos_users) INTO v_is_liked
  FROM matches WHERE id = p_match_id;

  IF v_is_liked THEN
    UPDATE matches
    SET kudos_users = array_remove(kudos_users, v_player_id::text),
        kudos_count = GREATEST(0, kudos_count - 1)
    WHERE id = p_match_id;
  ELSE
    UPDATE matches
    SET kudos_users = array_append(kudos_users, v_player_id::text),
