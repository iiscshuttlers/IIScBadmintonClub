...
CREATE POLICY "Scorer can insert"
  ...
  WITH CHECK (auth.uid() = (SELECT user_id FROM public.players WHERE id = scorer_id));

CREATE POLICY "Scorer can update"
  ON public.live_matches FOR UPDATE
  USING (auth.uid() = (SELECT user_id FROM public.players WHERE id = scorer_id));
...
CREATE TABLE IF NOT EXISTS public.match_predictions (
  ...
  predicted_winner_id  TEXT        NOT NULL REFERENCES public.players(id) ON DELETE CASCADE,
