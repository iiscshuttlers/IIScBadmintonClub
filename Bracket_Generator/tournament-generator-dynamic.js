 ...
 * Uses actual player names, departments, and seeding from Excel file
 */
...
function processDoublesData(rawData) {
  return rawData
    ...
    .map(row => {
      const playerA = (row.Team || '').toString().trim();
      const deptA = (row.__EMPTY || '').toString().trim();
      const playerB = (row.__EMPTY_1 || '').toString().trim();
      const deptB = (row.__EMPTY_2 || '').toString().trim();
...
function createSinglesRound1(format, players) {
  ...
  players.forEach(p => { participantMap[p.seed] = p; });
...
function createDoublesRound1(format, teams) {
  ...
  let matchNum = 1;
  const participantMap = {};
  teams.forEach(t => { participantMap[t.seed] = t; });
  ...
  for (let i = 0; i < pattern.length; i += 2) {
    ...
    if (seed1 === 'BYE' || !participantMap[seed1]) {
      ...
      players2 = t2 ? `${t2.player1}/${t2.player2} (${t2.department1}/${t2.department2})` : 'Bye';
    } else if (seed2 === 'BYE' || !participantMap[seed2]) {
      const t1 = participantMap[seed1];
      players1 = t1 ? `${t1.player1}/${t1.player2} (${t1.department1}/${t1.department2})` : 'Bye';
      players2 = 'Bye';
    } else {
      const t1 = participantMap[seed1];
      const t2 = participantMap[seed2];
    ...
    const match = {
      Match_ID: `${format}_${matchNum}`,
      Round: 'Round 1',
      Status: (players1 === 'Bye' || players2 === 'Bye') ? 'completed' : 'scheduled',
      Score_1: '',
      Winner: '',
      Players_1: players1,
      Players_2: players2
...
function generateFormatMatches(format, participants, isDoubles) {
  ...
  const laterRounds = createSubsequentRounds(format, round1.length, matchNum, isDoubles);
...
function generateTournamentData(excelFilePath) {
  ...
  const totalMatches = Object.values(tournament.matches).reduce((sum, matches) => sum + matches.length, 0);
