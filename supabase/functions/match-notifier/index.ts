...
async function dispatchNotifications(supabase: any, match: any, tournament: any) {
  ...
  const projectId = Deno.env.get('FIREBASE_SERVICE_ACCOUNT')
  ...
  if (matchDateString === nowDateString) dateLabel = 'Today';
    ...
    dateLabel = matchDate.toLocaleDateString('en-IN', {
      timeZone: 'Asia/Kolkata',
      weekday: 'short',
      month: 'short',
      day: 'numeric',
  ...
  const timeString = matchDate.toLocaleTimeString('en-IN', {
    timeZone: 'Asia/Kolkata',
    hour: '2-digit',
    minute: '2-digit',
  });
  ...
  for (const pId of playerIds) {
    ...
    if (isUmpire) {
      ...
      body =
        opponentLabel && opponentLabel !== 'TBD' && opponentLabel !== 'BYE'
...
async function dispatchFanNotifications(
  supabase: any,
  ...
  const { data: futureMatches } = await supabase
    ...
    .select(
      'id, match_code, category, scheduled_at, team1_label, team2_label, player1_id, player2_id, player3_id, player4_id'
    )
  ...
  for (const match of futureMatches) {
    ...
    for (const sub of pSubs) {
      if (minsUntilMatch <= sub.notify_before_mins && minsUntilMatch >= 0) {
        if (
          !usersToNotify.has(sub.user_id) ||
          usersToNotify.get(sub.user_id)! < sub.notify_before_mins
        ) {
          usersToNotify.set(sub.user_id, sub.notify_before_mins);
        }
      }
  ...
  if (sentRecords.length > 0) {
    await supabase
      .from('sent_fan_notifications')
      .insert(sentRecords)
      .catch((e: any) => console.error('Failed to insert sent fan notifs', e));
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'manual' && match_id) {
      ...
      return new Response(JSON.stringify({ message: 'Reminder sent manually!' }), {
        headers: corsHeaders,
        status: 200,
      });
    } else {
      ...
      if (!tournaments || tournaments.length === 0) {
        return new Response(JSON.stringify({ message: 'No active auto-reminder tournaments' }), {
          headers: corsHeaders,
        });
      }
      ...
      return new Response(
        JSON.stringify({ message: `Sent reminders for ${matches ? matches.length : 0} matches` }),
        { headers: corsHeaders }
      );
    }
  } catch (err: any) {
