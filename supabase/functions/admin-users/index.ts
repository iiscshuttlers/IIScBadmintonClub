...
serve(async (req) => {
  ...
  try {
    ...
    // Verify admin status
    const adminEmails = ['janmejayk18@gmail.com', 'admin@iiscshuttlers.com', 'shuttlersiisc@gmail.com', 'paras@iiscshuttlers.com'];
    const isAdmin = adminEmails.includes(user.email ?? '');

    if (!isAdmin) {
      // Check if they are in site_data roles
      const { data: rolesData } = await supabaseAdmin.from('site_data').select('value').eq('key', 'roles').single()
