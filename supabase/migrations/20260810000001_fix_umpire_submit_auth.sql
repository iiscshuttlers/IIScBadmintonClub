-- Fix umpire submit auth to also check umpired_by, and enforce 10-minute edit limit
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM players WHERE id = auth.uid() AND role IN ('admin','master_admin','umpire')) 
     AND auth.uid() != v_match.umpired_by
  THEN
    RAISE EXCEPTION 'Unauthorized';
  END IF;

  IF v_match.status = 'completed' AND v_match.scored_at IS NOT NULL THEN
    IF EXTRACT(EPOCH FROM (NOW() - v_match.scored_at)) > 600 THEN
...
BEGIN
  ...
  ) AND auth.uid() != v_umpired_by THEN
    RAISE EXCEPTION 'Unauthorized';
  END IF;
