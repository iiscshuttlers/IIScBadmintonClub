-- Fix typos in recent migrations that cause recalculate and trigger to fail

-- 1. Fix 'is_approved' typo in protect_player_sensitive_columns
...
-- 2. Fix 'config' typo in recalculate_tournament_elo
  ...
  LOOP
    ...
    IF v_is_doubles OR v_is_mixed THEN
      IF m_record.team1_partner_id IS NOT NULL THEN
        ...
        FROM players WHERE id = m_record.team1_partner_id;
      ...
      IF m_record.team2_partner_id IS NOT NULL THEN
        ...
        FROM players WHERE id = m_record.team2_partner_id;
    ...
    v_s_mult := get_sets_multiplier(m_record.team1_sets, m_record.team2_sets);
    v_d_mult := get_dominance_multiplier(m_record.team1_scores, m_record.team2_scores, team1_actual = 1);
